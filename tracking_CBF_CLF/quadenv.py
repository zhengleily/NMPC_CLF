...
class Quad():
    ...
    #     if t >= 600*0.02:
    #         v = [
    #             -2.5 + 2 * np.cos((x - 1) * 0.86 / 20) * 0.86 / 1 + 5,
    #             2.5 - 2 * np.cos((y - 5) * 0.86 / 20) * 0.86 / 1 + 5,
    #             0.5 + 3,
    #         ]
    #     if t >= 300*0.02 and t <= 600*0.02:
    #         v = [
    #             -2.5 + 2 * np.cos((x - 1) * 0.86 / 20) * 0.86 / 1,
    #             2.5 - 2 * np.cos((y - 5) * 0.86 / 20) * 0.86 / 1,
    #             0.5,
    #         ]
    #     else:
    #         v = [
    ...
    def set_angle(self, state_angle):
        [self.phi, self.theta, self.psi] = state_angle
...
class quad_env():
    def __init__(self):
        ...
        self.action_dim = 4
        '''line'''
        self.curve_pos = lambda t: np.array([0, 0.5 * t,  t/(t + 1)])
        self.curve_vel = lambda t: np.array([0, 0.5, 1/(t+1)**2])
        self.curve_accel_0 = np.array([-1, 0, 0])
        '''curved'''
        self.curve_pos = lambda t: np.array([-2 * np.sin(0.5 * t), 5 + 2 * np.cos(0.5 * t), 0.4 * t + 1])
        self.curve_vel = lambda t: np.array([-np.cos(0.5 * t), -np.sin(0.5 * t), 0 * t+0.4])
        self.curve_accel_0 = np.array([-1,0, 0])
        ''' LL'''
        ...
        #     pts = scale * pts + center[:, np.newaxis]
        #     self.obstacles.append(pts)
        '''junxun code - solution of errors'''
        try:
            pass
        except type as e:
            print("1")
